...
*
* 해당 문제는 1024MB, 12초 라는 자주 보기 힘든 넉넉한 메모리와 시간 제한을 제공하지만 문제가 브루트포스로 O(N^4)의 시간복잡도를 지닌다는 사실을 알 수 있게 됩니다.
* 당연히 그 정도의 시간 복잡도는 허용되지 않기 때문에 좀 더 시간을 줄이도록 중간에서 만나기 (Meet in the middle) 기법을 사용해야하는 문제입니다.
*
* 중간에서 만나기는 분할 정복과 비슷한 개념입니다.
* 분할 정복은 커다란 문제를 나눌 수 없는 단위까지 작게 나눠서 각각을 풀이한 결과를 다시 합쳐서 문제를 해결하는 방식입니다.
* 이와 비슷하게 중간에서 만나기도 커다란 문제를 나눠서 각각 나눈 문제의 결과를 도출하고 이를 다시 이용하는 방식입니다.
*
