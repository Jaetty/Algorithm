...
public class BJ_게리맨더링_17471 {
	...
	public static void main(String[] args) throws Exception{
		...
		matrix = new int[N+1][N+1]; // 0 dummy <= 인구 수 저장해서 활용
		...
		for(int i=0; i<=N; i++) {
			...
			for(int j=1; j<=n; j++) {
				matrix[i][j] = Integer.parseInt(st.nextToken()); // i 구역에 인접한 구역 번호 (정점)
			}
		}
	...
	static void check() {
		...
		if(queue.size() == 0 ) return; // size==N 확인 X
		...
		while(!queue.isEmpty()) {
			...
			for(int i=1; i<=N; i++) {
				int adj = matrix[v][i];
				if(adj != 0 && !visit[adj] && select[adj]) { // 연결되어 있고, 아직 방문하지 않았고, B 그룹에 속하는 (select[adj] true)
					visit[adj] = true; // 연결된 정점이 true 로 표시된다.
					queue.offer(adj);
				}
			}
